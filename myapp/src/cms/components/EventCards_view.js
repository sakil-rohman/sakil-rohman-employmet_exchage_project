...
function EventCards_view() {
 ...
 const getSingleAboutUs = async () =>{
        const {data} = await axios.get(`http://127.0.0.1:8000/api/eventcards/${id}`)
        console.log(data)
        setAboutUs(data)
    }
    useEffect(() =>{
    ...
    const deleteAboutUs = async (id) =>{
      await axios.delete(`http://127.0.0.1:8000/api/eventcards/${id}/`)
      ...
      <tbody>
      {
       aboutuss.map((aboutus, index) => (
        <tr>
          ...
          <td>
          <img src={aboutus.image} style={{ width: '50px', height: '50px' }}className='rounded-circle' />
          </td>
          <td>
          ...
          <Link className="btn btn-primary m-2" to={`/${aboutus.id}/eventcards-edit`}> <FontAwesomeIcon icon={faEdit} /> </Link>
...
export default EventCards_view