...
useEffect(() =>{
  ...
  const UpdateGuidelineInfo= async () =>{
    ...
    await axios({
        method: 'PUT',
        url: `http://127.0.0.1:8000/api/guideline/${id}/`,
        data: formField
    }).then((response) =>{
    ...
    return (
        ...
        <>
       <div>
        <h1>Update Guideline Details</h1>
        <div className='container'>
            <div className='form-group'>
                <div className='form-control'>                  
                    <input type="text" className="form-control form-control-1g" placeholder="Enter " name= "name" value={title} onChange={(e) => setTitle (e.target.value)} />
                </div>
            </div>
            <div className='form-group'>
                <div className='form-control'>                  
                    <input type="text" className="form-control form-control-1g" placeholder="Enter " name= "name" value={heading} onChange={(e) => setHeading (e.target.value)} />
                </div>
            </div>
            <div className='form-group'>
                ...
                <div className='form-group'>
                <button className='btn btn-success' onClick={UpdateGuidelineInfo}>Update</button>
            </div>
