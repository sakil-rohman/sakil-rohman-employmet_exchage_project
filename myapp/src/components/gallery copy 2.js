...
function Gallery() {
    const [sliderimages, setSliderimages] = useState([])
      ...
      const response = await axios.get('http://127.0.0.1:8000/api/galleryimage/')
    ...
    const getSingleMinistry = async () =>{
          const {data} = await axios.get(`http://127.0.0.1:8000/api/galleryimage/${id}`)
          console.log(data)
 ...
 {/* <Image src='/images/sskg.png' /> */}
       ...
       sliderimages.map((sliderimage, index) => (
                // <div  key={image.id} className="imageG-card">
                <div  className="imageG-card">
                    <a href={sliderimage.image}>
						<img className="imageG" src={sliderimage.image} alt="" />
				    </a>
